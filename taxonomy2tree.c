...
/*void	traverseCheck (struct treeNode* n)
{
	char buffer[255];
	long i = 0;
	if (n->beenhere>=currentLineID)
	{
		sprintf (buffer, "Cycle at line %d\n", currentLineID);
		reportError (buffer);
	}
	n->beenhere ++;
	
	for (i = 0; i<n->children->vLength; i++)
		traverseCheck (((struct treeNode**)n->children->vData)[i]);	
}*/
...
int main (int argc, const char * argv[]) 
{
	...
	traverseTree (treeFile,((struct treeNode**)globalTreeRoot->children->vData)[0],(maxTreeLevel>0)?(maxTreeLevel):0xfffffL,0);
	fclose (treeFile);
	return 0;
}
